...
# Stripping/blending the image  / stitching
...
for i in range(5, 0, -1):
    ...
    laplasian = cv2.subtract(gp_apple[i-1],gaussian_extended)
...
for i in range(5, 0, -1):
    ...
    laplasian = cv2.subtract(gp_orange[i-1],gaussian_extended)
...
for apple_lap, orange_lap in zip(lp_apple, lp_orange):
    ...
    laplasian = np.hstack ((apple_lap[:, 0:int(cols/2)], orange_lap[:, int(cols/2):] ))
